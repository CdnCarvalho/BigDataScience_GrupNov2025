...
def imprimir(df, qtd=None):

    df_print = df.with_columns(
        pl.col("VALOR PARCELA").map_elements(lambda x: f"{x:,.2f}")
    )

    if qtd:
        print(df_print.head(qtd))
    else:
        print(df_print)
...
try:
    ...
    print('\nOutliers Superiores:')
    print(30*'-')
    imprimir(df_outliers_superiores)

    print('\nOutliers Inferiores:')
    print(30*'-')
    imprimir(df_outliers_inferiores)
