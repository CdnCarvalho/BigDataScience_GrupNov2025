...
# Função para conectar no banco e executar a consulta
def consultar_banco(host, user, password, database, query):
...
try:
    # Chama a função
    df_usuarios = consultar_banco(host, user, password, database, query)
...
# Relacionando e processando as informações
try:
    # Relacionando com merge
    df_emprestimos_usuarios = pd.merge(
        df_usuarios, 
        df_emprestimos, 
        on='id_usuario'
    )
    ...
    print("\nEmpréstimos pelos Usuários:")
    print(df_emprestimos_usuarios[
            ['id_emprestimo', 'nome', 'cidade', 'data_emprestimo', 'valor']
        ]
    )

    # Filtrar usuários da cidade Curitiba
    df_cidade = df_emprestimos_usuarios[df_emprestimos_usuarios['cidade'] == 'Rio de Janeiro']
